...
function searchButton()
	...
	elseif researchTree then
		...
		searchListRepopulationCooldown = data.searchListRepopulationInterval
		widget.setText("title", "Listeden bir araştırmaya tıklayarak görünümü ona odakla")
...
function treePickButton()
	...
	if widget.active("treeList") then
		...
		if consumptionRules.currency then
			consumeText = '^red; Para/puan harcanacak'
		else
			consumeText = '^green; Para/puan harcanmayacak'
		end

		if consumptionRules.items then
			consumeText = consumeText..'\n^red; Eşya harcanacak'
		else
			consumeText = consumeText..'\n^green; Eşya harcanmayacak'
		end
		...
		searchListRepopulationCooldown = data.searchListRepopulationInterval
		widget.setText("title", "Bir araştırma ağacı seç")
		widget.setVisible("researchButton", false)
		widget.setVisible("consumptionText", false)
	...
	if not widget.active("searchList") and not widget.active("treeList") then
		...
		if selected then
			if data.strings.research[selected] then
				...
				widget.setText("infoList.text", "HATA - Seçili araştırmanın metin verisi eksik")
			end
...
function populateTreeList()
	...
	if (#toSort == 0) then
		...
		widget.setText(listItem..".title", '^red;Kullanılabilir araştırma ağacı yok')
...
function draw()
	...
	-- Draw "READ ONLY"
	if readOnly then
		canvas:drawText("SALT OKUNUR!", {position = {57, canvasSize[2]-2}}, 7, "#FF5E66F0")
	end
	...
	if researchTree then
		...
		canvas:drawText("ARAŞTIRMA AĞACI SEÇİLMEDİ", {position = {canvasSize[1]*0.5, canvasSize[2]*0.5}, horizontalAnchor = "mid", verticalAnchor = "mid"}, 20, "#FF5E66F0")
	end
		...
		if not tbl[5] or player.currency(tbl[1]) > 0 then
			...
			and mousePosition[2] >= startPoint[2]-1 and mousePosition[2] <= startPoint[2] + 8 then
				if tbl[1] == "essence" then
					canvas:drawText("Öz", {position = {startPoint[1] + 8 + 3, startPoint[2]}, verticalAnchor = "bottom"}, 7, tbl[2])
				elseif tbl[1] == "money" then
					canvas:drawText("Piksel", {position = {startPoint[1] + 8 + 3, startPoint[2]}, verticalAnchor = "bottom"}, 7, tbl[2])
				else
...
function verifyAcronims()
	...
	if missing == "" then
		...
		widget.setText("title", "^red;HATA:^reset; Araştırma kısaltmaları eksik")
		...
		canvas:drawText("HATA\nEKSİK KISALTMALAR", {position = {canvasSize[1]*0.5, canvasSize[2]*0.5}, horizontalAnchor = "mid", verticalAnchor = "mid"}, 20, "#FF5E66F0")
...
cheats = {
	...
	nostringsonme = function()
		readOnly = false
		widget.setText("researchButton", "Araştır")
