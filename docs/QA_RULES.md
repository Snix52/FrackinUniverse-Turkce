...
- Zırh kapsamına alınan `.head`, `.chest`, `.legs` ve `.back` dosyalarında aktif araştırma düğümü ile gerçek `.recipe` çıktısı birlikte doğrulanmalı. v0.25 için dokuz Kademe 1-2 düğümünden seçilen exact allowlist'teki 114 assette, v0.26 için altı Kademe 3 düğümünden seçilen 75 yeni assette yalnız `/shortdescription` ve `/description` çevrilebilir; `itemName`, stat hesapları, efekt listeleri, tooltip türü, kategori, tarif ve script alanları korunmalı. EPP düğümleri bu zırh diliminin dışında tutulmalı.
