...
_V050_MANIFEST = json.loads(
    Path(__file__).with_name('v050_translations.json').read_text(encoding='utf-8')
)
V050_BEEKEEPING_FIELDS = {
    (row['asset'], row['pointer']) for row in _V050_MANIFEST['translations']
}
if len(V050_BEEKEEPING_FIELDS) != len(_V050_MANIFEST['translations']):
    raise ValueError('v0.50 arıcılık manifestinde yinelenen alan var')

_V051_MANIFEST = json.loads(
    Path(__file__).with_name('v051_translations.json').read_text(encoding='utf-8')
)
V051_PLATFORM_FIELDS = {
    (row['asset'], row['pointer']) for row in _V051_MANIFEST['translations']
}
if len(V051_PLATFORM_FIELDS) != len(_V051_MANIFEST['translations']):
    raise ValueError('v0.51 platform manifestinde yinelenen alan var')

_V052_MANIFEST = json.loads(
    Path(__file__).with_name('v052_translations.json').read_text(encoding='utf-8')
)
V052_MATERIAL_FIELDS = {
    (row['asset'], row['pointer']) for row in _V052_MANIFEST['translations']
}
...
_V054_MANIFEST = json.loads(
    Path(__file__).with_name('v054_translations.json').read_text(encoding='utf-8')
)
V054_HONEY_FIELDS = {
    (row['asset'], row['pointer']) for row in _V054_MANIFEST['translations']
}
if len(V054_HONEY_FIELDS) != len(_V054_MANIFEST['translations']):
    raise ValueError('v0.54 bal malzemeleri manifestinde yinelenen alan var')

_V055_MANIFEST = json.loads(
    Path(__file__).with_name('v055_translations.json').read_text(encoding='utf-8')
)
V055_STRUCTURE_FIELDS = {
    (row['asset'], row['pointer']) for row in _V055_MANIFEST['translations']
}
if len(V055_STRUCTURE_FIELDS) != len(_V055_MANIFEST['translations']):
    raise ValueError('v0.55 yapı malzemeleri manifestinde yinelenen alan var')

_V056_MANIFEST = json.loads(
    Path(__file__).with_name('v056_translations.json').read_text(encoding='utf-8')
)
V056_TILES_FIELDS = {
    (row['asset'], row['pointer']) for row in _V056_MANIFEST['translations']
}
...
_V058_MANIFEST = json.loads(
    Path(__file__).with_name('v058_translations.json').read_text(encoding='utf-8')
)
V058_CREATURE_FIELDS = {
    (row['asset'], row['pointer']) for row in _V058_MANIFEST['translations']
}
if len(V058_CREATURE_FIELDS) != len(_V058_MANIFEST['translations']):
    raise ValueError('v0.58 yaratık manifestinde yinelenen alan var')

_V059_MANIFEST = json.loads(
    Path(__file__).with_name('v059_translations.json').read_text(encoding='utf-8')
)
V059_MONSTER_FIELDS = {
    (row['asset'], row['pointer']) for row in _V059_MANIFEST['translations']
}
...
_V062_MANIFEST = json.loads(
    Path(__file__).with_name('v062_translations.json').read_text(encoding='utf-8')
)
V062_MECH_PART_FIELDS = {
    (row['asset'], row['pointer']) for row in _V062_MANIFEST['translations']
}
if len(V062_MECH_PART_FIELDS) != len(_V062_MANIFEST['translations']):
    raise ValueError('v0.62 mek parçaları manifestinde yinelenen alan var')

_V063_MANIFEST = json.loads(
    Path(__file__).with_name('v063_translations.json').read_text(encoding='utf-8')
)
V063_FOOD_FIELDS = {
    (row['asset'], row['pointer']) for row in _V063_MANIFEST['translations']
}
if len(V063_FOOD_FIELDS) != len(_V063_MANIFEST['translations']):
    raise ValueError('v0.63 yiyecek manifestinde yinelenen alan var')

_V064_MANIFEST = json.loads(
    Path(__file__).with_name('v064_translations.json').read_text(encoding='utf-8')
)
V064_FOOD_FIELDS = {
    (row['asset'], row['pointer']) for row in _V064_MANIFEST['translations']
}
...
def allowed(a,p):
    ...
    if (a,p) in V063_FOOD_FIELDS:
        return True
    if (a,p) in V062_MECH_PART_FIELDS:
        return True
    ...
    if (a,p) in V051_PLATFORM_FIELDS:
        return True
    if (a,p) in V050_BEEKEEPING_FIELDS:
        return True
