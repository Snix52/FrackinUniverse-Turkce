#!/usr/bin/env python3
"""Exact-source gate for the v0.52 earth, stone and masonry materials tranche."""
...
MANIFEST = TOOLS / "v052_translations.json"
CATALOG = TOOLS / "ceviriler.json"
EXPECTED_FIELDS = 318
EXPECTED_ASSETS = 126
EXPECTED_UNIQUE = 282
INCLUDE = re.compile(r"(stone|rock|granite|slate|andesite|marble|cobble|brick|block|gravel|shale|clay|soil|sand|dirt|silt)", re.I)
EXCLUDE = re.compile(r"(metal|steel|iron|copper|gold|silver|titanium|uranium|plutonium|irradi|hull|window|pipe|wire|cable|support|pillar|beam|girder|floor|platform|wall|panel|roof|fence|stairs)", re.I)
    ...
    for path in sorted(source.rglob("*")):
        ...
        for candidate in audit.candidates_from_data(rel.as_posix(), data):
            ...
            if candidate is None:
                continue
            stem = Path(candidate.asset).stem
            if (
                candidate.confidence != "confirmed"
                or not candidate.asset.startswith("tiles/materials/")
                or not INCLUDE.search(stem)
                or EXCLUDE.search(stem)
            ):
                continue
            if (
...
def main() -> None:
    ...
    if manifest["translation_version"] != "0.52.0-beta":
        raise ValueError("v0.52 manifest version drift")
    keys = {(row["asset"], row["pointer"]) for row in rows}
    if len(rows) != EXPECTED_FIELDS or len(keys) != EXPECTED_FIELDS:
        raise ValueError(f"v0.52 field count/duplicate drift: {len(rows)}")
    if len({row["asset"] for row in rows}) != EXPECTED_ASSETS:
        raise ValueError("v0.52 asset count drift")
    if len({row["en"] for row in rows}) != EXPECTED_UNIQUE:
        raise ValueError("v0.52 unique source count drift")
    ...
    if set(candidates) != keys:
        ...
        raise ValueError(f"v0.52 candidate set differs: missing={missing[:3]} extra={extra[:3]}")
    for row in rows:
        if candidates[(row["asset"], row["pointer"])].value != row["en"]:
            raise ValueError("v0.52 pinned source mismatch: " + row["asset"] + row["pointer"])

    catalog = json.loads(CATALOG.read_text(encoding="utf-8"))
    version = tuple(int(part) for part in catalog["translation_version"].split("-", 1)[0].split("."))
    if version < (0, 52, 0):
        raise ValueError("catalog version behind v0.52")
    ...
    for row in rows:
        ...
        if not current or current["en"] != row["en"] or current["tr"] != row["tr"]:
            raise ValueError("v0.52 catalog mismatch: " + row["asset"] + row["pointer"])

    print(f"v0.52 source gate PASS: {len(rows)} alan / {len({row['asset'] for row in rows})} asset / {len({row['en'] for row in rows})} kaynak")
