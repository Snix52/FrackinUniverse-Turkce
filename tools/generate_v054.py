#!/usr/bin/env python3
"""Exact-source gate for the v0.54 honey and golden-wood materials tranche."""
...
MANIFEST = TOOLS / "v054_translations.json"
CATALOG = TOOLS / "ceviriler.json"
EXPECTED_FIELDS = 71
EXPECTED_ASSETS = 18
EXPECTED_UNIQUE = 62
INCLUDE = re.compile(r"^honey", re.I)
    ...
    for path in sorted(source.rglob("*")):
        ...
        for candidate in audit.candidates_from_data(rel.as_posix(), data):
            ...
            if (
                ...
                or not (
                    candidate.asset.startswith("tiles/materials/honey/")
                    or INCLUDE.search(stem)
                )
            ):
                continue
            if (
...
def main() -> None:
    ...
    if manifest["translation_version"] != "0.54.0-beta":
        raise ValueError("v0.54 manifest version drift")
    keys = {(row["asset"], row["pointer"]) for row in rows}
    if len(rows) != EXPECTED_FIELDS or len(keys) != EXPECTED_FIELDS:
        raise ValueError(f"v0.54 field count/duplicate drift: {len(rows)}")
    if len({row["asset"] for row in rows}) != EXPECTED_ASSETS:
        raise ValueError("v0.54 asset count drift")
    if len({row["en"] for row in rows}) != EXPECTED_UNIQUE:
        raise ValueError("v0.54 unique source count drift")
    ...
    if set(candidates) != keys:
        ...
        raise ValueError(f"v0.54 candidate set differs: missing={missing[:3]} extra={extra[:3]}")
    for row in rows:
        if candidates[(row["asset"], row["pointer"])].value != row["en"]:
            raise ValueError("v0.54 pinned source mismatch: " + row["asset"] + row["pointer"])

    catalog = json.loads(CATALOG.read_text(encoding="utf-8"))
    version = tuple(int(part) for part in catalog["translation_version"].split("-", 1)[0].split("."))
    if version < (0, 54, 0):
        raise ValueError("catalog version behind v0.54")
    ...
    for row in rows:
        ...
        if not current or current["en"] != row["en"] or current["tr"] != row["tr"]:
            raise ValueError("v0.54 catalog mismatch: " + row["asset"] + row["pointer"])

    print(f"v0.54 source gate PASS: {len(rows)} alan / {len({row['asset'] for row in rows})} asset / {len({row['en'] for row in rows})} kaynak")
