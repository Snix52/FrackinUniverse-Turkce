#!/usr/bin/env python3
"""Exact-source gate for the v0.55 metal, timber, glass and window materials tranche."""
...
MANIFEST = TOOLS / "v055_translations.json"
CATALOG = TOOLS / "ceviriler.json"
EXPECTED_FIELDS = 466
EXPECTED_ASSETS = 177
EXPECTED_UNIQUE = 393
INCLUDE = re.compile(r"metal|metallic|iron|steel|tungsten|copper|tethyde|impervium|aether|nightar|avali|avian|kirhos|precursor|skath|hull|ship|tech|pipe|cable|girder|grate|industrial|wood|bamboo|timber|plank|shingle|shoji|wallpaper|lattice|window|glass", re.I)
...
def main() -> None:
    ...
    if manifest["translation_version"] != "0.55.0-beta":
        raise ValueError("v0.55 manifest version drift")
    keys = {(row["asset"], row["pointer"]) for row in rows}
    if len(rows) != EXPECTED_FIELDS or len(keys) != EXPECTED_FIELDS:
        raise ValueError(f"v0.55 field count/duplicate drift: {len(rows)}")
    if len({row["asset"] for row in rows}) != EXPECTED_ASSETS:
        raise ValueError("v0.55 asset count drift")
    if len({row["en"] for row in rows}) != EXPECTED_UNIQUE:
        raise ValueError("v0.55 unique source count drift")
    ...
    if set(candidates) != keys:
        ...
        raise ValueError(f"v0.55 candidate set differs: missing={missing[:3]} extra={extra[:3]}")
    for row in rows:
        if candidates[(row["asset"], row["pointer"])].value != row["en"]:
            raise ValueError("v0.55 pinned source mismatch: " + row["asset"] + row["pointer"])

    catalog = json.loads(CATALOG.read_text(encoding="utf-8"))
    version = tuple(int(part) for part in catalog["translation_version"].split("-", 1)[0].split("."))
    if version < (0, 55, 0):
        raise ValueError("catalog version behind v0.55")
    ...
    for row in rows:
        ...
        if not current or current["en"] != row["en"] or current["tr"] != row["tr"]:
            raise ValueError("v0.55 catalog mismatch: " + row["asset"] + row["pointer"])

    print(f"v0.55 source gate PASS: {len(rows)} alan / {len({row['asset'] for row in rows})} asset / {len({row['en'] for row in rows})} kaynak")
