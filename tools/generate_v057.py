#!/usr/bin/env python3
"""Exact-source gate for the v0.57 walker and flyer monster tranche."""
...
MANIFEST = TOOLS / "v057_translations.json"
CATALOG = TOOLS / "ceviriler.json"
EXPECTED_FIELDS = 477
EXPECTED_ASSETS = 239
EXPECTED_UNIQUE = 344
DEBUG_FIELDS = {
    ("monsters/walkers/spheresnaketest/spheresnaketesthead.monstertype", "/description"),
    ("monsters/walkers/spheresnaketest/spheresnaketesthead.monstertype", "/shortdescription"),
    ("monsters/walkers/spheresnaketest/spheresnaketestsegment.monstertype", "/description"),
    ("monsters/walkers/spheresnaketest/spheresnaketestsegment.monstertype", "/shortdescription"),
    ("monsters/walkers/spheretest/spheretest.monstertype", "/description"),
    ("monsters/walkers/spheretest/spheretest.monstertype", "/shortdescription"),
}
    ...
    for path in sorted(source.rglob("*")):
        ...
        for candidate in audit.candidates_from_data(rel.as_posix(), data):
            ...
            if candidate is None:
                continue
            if candidate.confidence != "confirmed" or not candidate.asset.startswith(("monsters/walkers/", "monsters/flyers/")):
                continue
            if (
...
def main() -> None:
    ...
    if manifest["translation_version"] != "0.57.0-beta":
        raise ValueError("v0.57 manifest version drift")
    keys = {(row["asset"], row["pointer"]) for row in rows}
    if len(rows) != EXPECTED_FIELDS or len(keys) != EXPECTED_FIELDS:
        raise ValueError(f"v0.57 field count/duplicate drift: {len(rows)}")
    if len({row["asset"] for row in rows}) != EXPECTED_ASSETS:
        raise ValueError("v0.57 asset count drift")
    if len({row["en"] for row in rows}) != EXPECTED_UNIQUE:
        raise ValueError("v0.57 unique source count drift")
    ...
    if set(candidates) != keys:
        ...
        raise ValueError(f"v0.57 candidate set differs: missing={missing[:3]} extra={extra[:3]}")
    for row in rows:
        if candidates[(row["asset"], row["pointer"])].value != row["en"]:
            raise ValueError("v0.57 pinned source mismatch: " + row["asset"] + row["pointer"])

    catalog = json.loads(CATALOG.read_text(encoding="utf-8"))
    version = tuple(int(part) for part in catalog["translation_version"].split("-", 1)[0].split("."))
    if version < (0, 57, 0):
        raise ValueError("catalog version behind v0.57")
    ...
    for row in rows:
        ...
        if not current or current["en"] != row["en"] or current["tr"] != row["tr"]:
            raise ValueError("v0.57 catalog mismatch: " + row["asset"] + row["pointer"])

    print(f"v0.57 source gate PASS: {len(rows)} alan / {len({row['asset'] for row in rows})} asset / {len({row['en'] for row in rows})} kaynak")
