#!/usr/bin/env python3
"""Exact-source gate for the v0.62 mech parts tranche."""
...
MANIFEST = TOOLS / "v062_translations.json"
...
def source_candidates(source: Path) -> dict[tuple[str, str], audit.Candidate]:
    ...
    for path in sorted((source / "items" / "generic" / "mechparts").rglob("*")):
        ...
        for candidate in audit.candidates_from_data(rel.as_posix(), data):
            ...
            if candidate is None or candidate.confidence != "confirmed" or not candidate.asset.startswith("items/generic/mechparts/"):
                continue
...
def main() -> None:
    ...
    if manifest["translation_version"] != "0.62.0-beta":
        raise ValueError("v0.62 manifest version drift")
    keys = {(row["asset"], row["pointer"]) for row in rows}
    if len(rows) != EXPECTED_FIELDS or len(keys) != EXPECTED_FIELDS:
        raise ValueError(f"v0.62 field count/duplicate drift: {len(rows)}")
    if len({row["asset"] for row in rows}) != EXPECTED_ASSETS:
        raise ValueError("v0.62 asset count drift")
    if len({row["en"] for row in rows}) != EXPECTED_UNIQUE:
        raise ValueError("v0.62 unique source count drift")
    ...
    if set(candidates) != keys:
        ...
        raise ValueError(f"v0.62 candidate set differs: missing={missing[:3]} extra={extra[:3]}")
    for row in rows:
    ...
    if version < (0, 62, 0):
        raise ValueError("catalog version behind v0.62")
    ...
    for row in rows:
        ...
        if not current or current["en"] != row["en"] or current["tr"] != row["tr"]:
            raise ValueError("v0.62 catalog mismatch: " + row["asset"] + row["pointer"])

    print(f"v0.62 source gate PASS: {len(rows)} alan / {len({row['asset'] for row in rows})} asset / {len({row['en'] for row in rows})} kaynak")
