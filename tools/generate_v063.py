#!/usr/bin/env python3
"""Exact-source gate for the v0.63 generic food tranche."""
...
MANIFEST = TOOLS / "v063_translations.json"
...
def main() -> None:
    ...
    if manifest["translation_version"] != "0.63.0-beta":
        raise ValueError("v0.63 manifest version drift")
    keys = {(row["asset"], row["pointer"]) for row in rows}
    if len(rows) != EXPECTED_FIELDS or len(keys) != EXPECTED_FIELDS:
        raise ValueError(f"v0.63 field count/duplicate drift: {len(rows)}")
    if len({row["asset"] for row in rows}) != EXPECTED_ASSETS:
        raise ValueError("v0.63 asset count drift")
    if len({row["en"] for row in rows}) != EXPECTED_UNIQUE:
        raise ValueError("v0.63 unique source count drift")
    ...
    if set(candidates) != keys:
        ...
        raise ValueError(f"v0.63 candidate set differs: missing={missing[:3]} extra={extra[:3]}")
    for row in rows:
        if candidates[(row["asset"], row["pointer"])].value != row["en"]:
            raise ValueError("v0.63 pinned source mismatch: " + row["asset"] + row["pointer"])
        if re.search(r"\bmech\b", row["en"], re.I) and not re.search(r"\bMech\b", row["tr"]):
            raise ValueError("v0.63 LOCKED Mech term missing: " + row["asset"] + row["pointer"])

    catalog = json.loads(CATALOG.read_text(encoding="utf-8"))
    version = tuple(int(part) for part in catalog["translation_version"].split("-", 1)[0].split("."))
    if version < (0, 63, 0):
        raise ValueError("catalog version behind v0.63")
    ...
    for row in rows:
        ...
        if not current or current["en"] != row["en"] or current["tr"] != row["tr"]:
            raise ValueError("v0.63 catalog mismatch: " + row["asset"] + row["pointer"])

    print(f"v0.63 source gate PASS: {len(rows)} alan / {len({row['asset'] for row in rows})} asset / {len({row['en'] for row in rows})} kaynak")
