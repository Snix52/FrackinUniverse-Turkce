...
function update(dt)
	...
	if not initialized then
		...
		if widget.active("playerPixels") then
			widget.setText("playerPixels", "Piksellerin: "..tostring(player.currency("money")))
		end
...
function populateGoodsList()
	...
	for _, data in ipairs(stationData.goods) do
		...
		widget.setText(listItem..".sellStockLabel", "Stoğun:         "..player.currency(data.name))
...
function tradeA()
	if stationData.selected then
		...
		textTyper.init(textData, "HATA - 'tradeA' işlevinde eşya seçilmeden takas yapılmaya çalışıldı")
	end
end
...
function tradeB()
	if stationData.selected then
		...
		textTyper.init(textData, "HATA - 'tradeB' işlevinde eşya seçilmeden takas yapılmaya çalışıldı")
	end
end
