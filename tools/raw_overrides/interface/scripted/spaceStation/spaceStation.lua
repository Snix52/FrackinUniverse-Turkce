...
function init()
	...
	if chosenObject then
		...
		widget.setText("text", "HATA - İstasyon nesnesi bulunamadı")
	end
...
function update(dt)
	...
	if not initialized then
		...
		if widget.active("playerPixels") then
			widget.setText("playerPixels", "Pikselin: "..tostring(player.currency("money")))
		end
...
function populateGoodsList()
	...
	for _, data in ipairs(stationData.goods) do
		...
		widget.setText(listItem..".stationStockLabel", "İstasyon stoğu: "..stock)
		widget.setText(listItem..".sellStockLabel", "Stokun:     "..player.currency(data.name))
...
function sellGoods()
	if stationData.selected then
		...
		textTyper.init(textData, "Hiçbir eşya seçilmedi.\nBunu nasıl becerdin lan?\nCidden etkilendim.\n\nSanırım bunu raporla?")
	end
end
...
function tradeA()
	if stationData.selected then
		...
		textTyper.init(textData, "HATA - 'tradeA' işlevinde eşya seçili değilken takas denenmiş")
	end
end
...
function tradeB()
	if stationData.selected then
		...
		textTyper.init(textData, "HATA - 'tradeB' işlevinde eşya seçili değilken takas denenmiş")
	end
end
...
function specialsTableInit()
	...
	widget.setText("investLevel", "Sv. "..tostring(objectData.specialsTable.investLevel))
	...
	if objectData.specialsTable.investLevel < stationData.trading.investMaxLevel then
		widget.setText("investRequired", "Gerekli: "..tostring(objectData.specialsTable.investRequired - objectData.specialsTable.invested))
		...
		widget.setText("investRequired", "Tamamen yükseltildi!")
